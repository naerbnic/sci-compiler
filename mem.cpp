...
class MemListList {
 public:
  ...
  ~MemListList() {
    for (int i = 0; i < MemListList::nLists; i++) delete lists[i];
    active = False;
  }

  MemList* operator[](int i) { return lists[i]; }
...
void* operator new(size_t n) {
  ...
  ++newCalls;
  if (writeMemSizes && n < 32) ++memSizes[n];

  void* mp = 0;

  // Search the fixed size memory lists to see if the requested size
  // should be allocated from one of them.
  for (int i = 0; lists.active && lists[i] && !mp && i < MemListList::nLists;
       ++i)
    if (n <= lists[i]->size) mp = lists[i]->get();
...
void operator delete(void* mp) {
  ...
  if (i >= MemListList::nLists) free(mp);
}
...
void* MemList::operator new(size_t size) { return malloc(size); }

void MemList::operator delete(void* ptr) {
  if (ptr) free(ptr);
}
...
void MemBlk::init(MemList* bp, size_t s, uint32_t n) {
  ...
  char* mp;
  char* np;
  for (mp = head, np = Normalize(mp + s); LessP(np, end);
...
void* MemBlk::operator new(size_t s) {
  void* mem = malloc(s);
  if (!mem) Fatal("Out of memory. Attempted to allocate %u bytes.", s);
  return mem;
}

void MemBlk::operator delete(void* ptr) {
  if (ptr) free(ptr);
}

MemBlk::~MemBlk() { free(start); }
...
EMSMemList::EMSMemList(size_t s)
    :
  ...
  ems = new EMSBlock;
  if (ems && (numBlks = ems->size / s)) new EMSMemBlk(this, size, numBlks);
}

EMSMemList::~EMSMemList() { delete ems; }
...
void WriteMemSizes() {
  ...
  for (int i = 0; i < MemListList::nLists; ++i) lists[i]->display();
}
...
static ulong HeapFree(int (*fp)(heapinfo*), _HeapInfo* hp) {
  ...
  while (fp(&hi) == _HEAPOK) {
    if (!hi.in_use) {
      ...
      if (hp) {
        ++hp->count;
        if (hi.size > hp->largest) hp->largest = hi.size;
      }
    }
  }
