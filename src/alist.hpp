...
class AListIter {
 ...
 public:
  ...
  std::unique_ptr<ANOpCode> remove();
  ANOpCode* replaceWith(std::unique_ptr<ANOpCode> nn);
 ...
 private:
  friend struct AOpList;
  AListIter(TList<ANOpCode>::iterator iter) : iter_(std::move(iter)) {}
  TList<ANOpCode>::iterator iter_;
};
...
struct AList {
  ...
  TList<ANode>::iterator iter() { return list_.begin(); }
...
struct AOpList {
  ...
  void addBefore(ANOpCode* ln, std::unique_ptr<ANOpCode> nn) {
    list_.findIter(ln).addBefore(std::move(nn));
  }
  ...
  template <class T, class... Args>
  T* newNodeBefore(ANOpCode* before, Args&&... args) {
    auto node = std::make_unique<T>(std::forward<Args>(args)...);
    auto node_ptr = node.get();
    list_.findIter(before).addBefore(std::move(node));
    return node_ptr;
