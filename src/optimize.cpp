  ...
  for (ANOpSign* an = (ANOpSign*)al->first(); an; an = (ANOpSign*)al->next()) {
    ...
    switch (op) {
      ...
      case op_pushi: {
        int val = an->value;
        if (val == 0) {
          al->replaceWith(an, curList->newNode<ANOpCode>(op_push0));
          ++nOptimizations;

        } else if (val == 1) {
          al->replaceWith(an, curList->newNode<ANOpCode>(op_push1));
          ++nOptimizations;

        } else if (val == 2) {
          al->replaceWith(an, curList->newNode<ANOpCode>(op_push2));
          ++nOptimizations;

        } else if (accType == IMMEDIATE && accVal == val) {
          // If accumulator already contains this value,
          // just push it.
          al->replaceWith(an, curList->newNode<ANOpCode>(op_push));
          ++nOptimizations;

        } else if (stackType == IMMEDIATE && stackVal == val) {
          // If stack already contains this value, dup it.
          al->replaceWith(an, curList->newNode<ANOpCode>(op_dup));
          ++nOptimizations;
        }
      ...
      case op_ret:
        ...
        if (al->removeOp(op_ret)) ++nOptimizations;
        break;
      ...
      case op_loadi:
        if (al->removeOp(op_push)) {
          ...
          al->replaceWith(an, curList->newNode<ANOpSign>(op, stackVal));
          ++nOptimizations;
      ...
      case op_selfID:
        if (al->removeOp(op_push)) {
          ...
          if (rn) {
            an = (ANOpSign*)al->replaceWith(an, curList->newNode<ANSend>(op_self));
