...
Boolean ReadDebugFile() {
  ...
  while (fgets(buf, sizeof buf, fp)) {
    ...
    Symbol* s;
    Object* o;
    if ((s = classSymTbl->lookup(cp)) && (o = s->obj)) {
      cp = strtok(0, delims);
      if (!cp) {
...
void WriteDebugFile() {
  ...
  for (Symbol* s = classSymTbl->firstSym(); s; s = classSymTbl->nextSym()) {
    Object* c = s->obj;
    //	do we have any debug info for this class?
    if (c->fullFileName)
      fprintf(fp, "%s, %s, %lu, %lu\n", s->name, c->fullFileName, c->srcStart,
              c->srcEnd);
  }

  if (fclose(fp) == EOF) Panic("Error writing %s", fileName);
}
...
Boolean GetClassSource(char* className, char* dest) {
  ...
  Object* c = sym->obj;
  if (!c || !c->fullFileName) return False;
