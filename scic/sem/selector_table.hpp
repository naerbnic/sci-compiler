 ...
 public:
  class Builder {
   public:
    ...
    virtual status::Status DeclareSelector(ast::TokenNode<util::RefStr> name,
                                           SelectorNum selector_num) = 0;
    virtual status::Status AddNewSelector(
        ast::TokenNode<util::RefStr> name) = 0;
    virtual status::StatusOr<std::unique_ptr<SelectorTable>> Build() = 0;
  };
